...
def reDraw(tolS, tolN):
    ...
    if chkBtnVar.get():  # 检查选框model tree是否被选中
        if tolN < 2: tolN = 2
        myTree = re.createTree(reDraw.rawDat, tr.modelLeaf, tr.modelErr, (tolS, tolN))
        yHat = si.createForeCast(myTree, reDraw.testDat, si.modelTreeEval)
    else:
        myTree = re.createTree(reDraw.rawDat, ops=(tolS, tolN))
        yHat = si.createForeCast(myTree, reDraw.testDat)
